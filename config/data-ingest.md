# DATA-INGEST: Óscar Martínez Zamora — Corpus de Trayectoria Profesional y Académica

> **INSTRUCCIÓN AL LLM — LECTURA OBLIGATORIA ANTES DE PROCESAR ESTE CORPUS**
>
> Este documento contiene dos tipos de contenido claramente diferenciados:
>
> 1. **Contenido de trayectoria** — información factual sobre formación, experiencia, proyectos y habilidades de Óscar Martínez Zamora. Es el material que debe utilizarse para generar currículums, cartas de presentación y cualquier otro documento profesional.
>
> 2. **Meta-instrucciones** — indicaciones sobre cómo usar, priorizar, combinar o presentar el contenido de trayectoria. Están marcadas con la etiqueta `[META-INSTRUCCIÓN]` al final de cada experiencia. **Las meta-instrucciones nunca deben reproducirse en ningún documento dirigido a terceros. Son instrucciones para el LLM, no contenido del candidato.**

___
## SECCIÓN 0 — IDENTIDAD Y CONTACTO

**Nombre:** Óscar Martínez Zamora
**Residencia actual:** 1 rue Joliot Curie, Gif-sur-Yvette, 91190, Francia (campus de CentraleSupélec)
**Teléfono España (WhatsApp):** +34 608 337 872
**Teléfono Francia (llamadas):** +33 0765 506 485
**Correo institucional:** oscar.martinez-zamora@student-cs.fr
**Correo personal:** oscar.martnz1217@gmail.com
**LinkedIn:** https://www.linkedin.com/in/oscarmartinezzamora/

---
## SECCIÓN 1 — FORMACIÓN ACADÉMICA

---

### Diplôme d'Ingénieur Généraliste — CentraleSupélec, Université Paris-Saclay

**Tipo:** Formación académica — título de ingeniería de Grande École
**Período:** septiembre 2025 – abril 2027
**Ubicación:** Gif-sur-Yvette (campus), París, Francia

**Especialización:** Operations Research and Risk Analytics (ORRA) — programa íntegramente en inglés
**Ejes temáticos de la especialización:**

- _Investigación operativa:_ optimización lineal y no lineal, programación dinámica, procesos estocásticos, heurísticas, teoría de grafos, simulación
- _Análisis de riesgo:_ teoría de la fiabilidad, estadística aplicada, machine learning, gestión de riesgos sociotécnicos
- _Orientación general:_ toma de decisiones bajo incertidumbre en sistemas industriales complejos — mantenimiento, resiliencia de infraestructuras, optimización de operaciones, modelado de riesgo

**Asignaturas representativas:** Stochastic Optimization for Decision under Uncertainty · Decision Making and Preference Modelling · Optimisation in Practice · Predictive Maintenance · Resilience of Critical Infrastructures · Data Analytics for Risk and Reliability

**Nota diferencial:** Primer estudiante del Máster en Ingeniería Industrial de la UPV en realizar este intercambio.

> **[META-INSTRUCCIÓN]** Esta nota diferencial debe incluirse en toda mención de CentraleSupélec. Es un marcador de iniciativa y carácter pionero que distingue el perfil.

---

### Máster en Ingeniería Industrial — Universitat Politècnica de València (UPV)

**Tipo:** Formación académica — máster de ingeniería
**Período:** septiembre 2024 – abril 2027
**Ubicación:** Valencia, España

**Nota media:** 7,7/10
**Rango:** Top 10% de la promoción
**Modalidad:** Cursado en simultáneo con el Diplôme d'Ingénieur de CentraleSupélec

---

### Grado en Ingeniería Industrial — Universitat Politècnica de València (UPV)

**Tipo:** Formación académica — grado universitario
**Período:** septiembre 2020 – julio 2024
**Ubicación:** Valencia, España

**Nota media:** 8,7/10
**Rango:** Top 2% de una promoción de 311 alumnos
**Distinción:** Premio de Alto Rendimiento Académico de la UPV por haber superado el 25% de los créditos con Matrícula de Honor (equivalente francés: Mention Très Bien)
**Movilidad internacional:** Créditos finales completados en Politechnika Krakowska (Cracovia, Polonia) durante el primer semestre de 2024, en inglés.

---

## SECCIÓN 2 — IDIOMAS

**Español:** Nativo
**Inglés:** C1 Advanced — certificado Cambridge
**Francés:** B2 — certificado DELF; en progresión hacia C1 (entorno académico y laboral cotidiano en CentraleSupélec)
**Alemán:** A2 — sin certificar

---

## SECCIÓN 3 — HABILIDADES TÉCNICAS

**Programación y datos:** Python, SQL, MATLAB

**Optimización e investigación operativa:** Gurobi, Simul8

**Análisis y business intelligence:** Power BI, Excel avanzado

**Diseño e ingeniería:** AutoCAD, Fusion 360, CYPE

**Documentación y presentación:** LaTeX, Suite Office (Word, PowerPoint, Excel)

**Gestión de proyectos:** Notion, MS Project

**Automatización y workflows asistidos por IA:** n8n, Claude Code, AI-assisted software development (generación y refactorización de código por LLM)

---

## SECCIÓN 4 — EXPERIENCIA PROFESIONAL

---

### Prácticas en ingeniería civil — Estudio de Ingeniería Bucarest 54

**Tipo:** Experiencia profesional — prácticas en empresa
**Período:** junio 2023 – agosto 2023
**Ubicación:** Murcia, España
**Empresa:** Bucarest 54
**Sector:** Ingeniería civil / despacho de ingeniería

**Responsabilidades y entregables:**

- Diseño completo de la infraestructura de un edificio residencial durante los tres meses de prácticas
- Esquema unifilar eléctrico (SLD) — diseño íntegro
- Instalación de fontanería y saneamiento — diseño íntegro
- Sistema de climatización (HVAC) — diseño íntegro
- Entrega completa del proyecto: planos, memoria técnica, anexos de cálculo y verificación de cumplimiento normativo según los códigos españoles de construcción (CTE)

**Herramientas:** CYPE (software profesional de ingeniería de instalaciones)

**Competencias demostradas:** Diseño técnico de instalaciones · Gestión autónoma de proyecto de ingeniería · Aplicación de normativa española de construcción · Documentación técnica profesional

---

## SECCIÓN 5 — VIDA ASOCIATIVA

---

### Secretario General y miembro del Pôle Relation Entreprises — Bureau de l'International (BDI), CentraleSupélec

**Tipo:** Vida asociativa — cargo de dirección en asociación de Grande École
**Período:** febrero 2026 – febrero 2027
**Ubicación:** Campus CentraleSupélec, Gif-sur-Yvette, Francia

#### Contexto de la organización

El Bureau de l'International (BDI) es la asociación referente del campus de CentraleSupélec en la integración de estudiantes internacionales. Asociación con presupuesto propio, gobernanza formal (Conseil d'Administration, statuts, Règlement Intérieur) y eventos de gran escala: semana de intégration (~200 participantes), World Week, galas y soirées durante el año. 35 miembros activos.

#### Rol 1 — Secretario General

Coordinador transversal de la asociación. Rol definido internamente como Human Resources Manager del Bureau.

**Responsabilidades operativas:**

- Diseño de planes de personal (staffing) para los grandes eventos: semana de intégration, World Week, soirées, galas
- Elaboración de roadmaps y coordinación del calendario del Bureau
- Redacción de actas de reuniones semanales (CRs) y procès-verbaux de los Conseils d'Administration
- Gestión de la comunicación interna multinivel: canal por pôle, canal Bureau, canal Important (anuncios oficiales), canal Direction (VPs + representante de internacionales)
- Contacto continuo con clubes nacionales dependientes del BDI y asociaciones del campus colaboradoras
- Negociación y representación de los intereses del BDI frente a otras asociaciones del campus
- Responsable del cumplimiento estatutario, la gestión documental y el mantenimiento del local
- Gestión del clima del equipo: detección temprana de señales de desmotivación o sobrecarga; reactivación de la implicación de miembros mediante reuniones específicas

#### Rol 2 — Miembro del Pôle Relation Entreprises

Gestión de las alianzas corporativas del BDI con tres partners de primer nivel:

- **McKinsey París** — interlocución y gestión de la relación corporativa
- **McKinsey Casablanca** — interlocución y gestión de la relación corporativa
- **Société Générale** — interlocución y gestión de la relación corporativa

**Responsabilidades adicionales del Pôle:**

- Gestión del merchandising de la asociación: relación con proveedores, selección de gama, gestión de pedidos
- Coordinación de la operación logística anual con IKEA: encuesta a estudiantes internacionales entrantes sobre mobiliario/utensilios necesarios, centralización del pedido y coordinación de la entrega para que coincida con su llegada al campus

#### Resultados cuantificados

- Asociación de 35 miembros activos coordinada operativamente de forma transversal
- 3 alianzas corporativas activas con firmas Tier-1 (McKinsey París, McKinsey Casablanca, Société Générale)
- Eventos coordinados con hasta 200 participantes (semaine d'intégration, World Week)
- Operación logística IKEA ejecutada anualmente
- Documentación institucional completa: CRs semanales, PVs de CA, gestión estatutaria

#### Competencias demostradas

Stakeholder management multinivel (miembros, Bureau, clubes nacionales, asociaciones, dirección escuela —DRI y DPIET—, partners corporativos) · Project management y operaciones de eventos a escala (+200 participantes) · B2B partnership management con firmas Tier-1 · Negociación y representación institucional · Gestión de equipos multiculturales · Crisis management y resolución de conflictos · Compliance y gobernanza asociativa (Loi 1901, statuts, RI, CA, procès-verbaux) · Supply chain básica y vendor management · Comunicación escrita rigurosa en francés · Liderazgo transversal sin autoridad jerárquica directa · Diplomacia · Escucha activa

> **[META-INSTRUCCIÓN]** Esta entrada debe aparecer siempre junto al rol de Tesorero del Club Español, y viceversa: juntos justifican la dimensión asociativa completa del perfil. En documentos donde se mencione la gestión del clima del equipo, los términos equivalentes en lenguaje de mercado son people management y team engagement.

---

### Tesorero (Trésorier) — Club Español de CentraleSupélec

**Tipo:** Vida asociativa — cargo de responsabilidad financiera en club cultural
**Período:** febrero 2026 – febrero 2027
**Ubicación:** Campus CentraleSupélec, Gif-sur-Yvette, Francia

#### Contexto de la organización

El Club Español integra y representa a la comunidad española del campus de CentraleSupélec. Opera bajo un modelo de financiación mixta: subvenciones del BDI (aprobadas en Conseil d'Administration) e ingresos propios de los eventos. El tesorero asume la responsabilidad legal y operativa de toda la actividad económica del club.

#### Responsabilidades

**1. Gestión financiera**

- Gestión de la cuenta bancaria asociativa en Société Générale
- Emisión de cheques de garantía (caution de 150 €)
- Facturación formal a nombre del BDI con número SIRET para todos los gastos subvencionados
- Conciliación ingresos-gastos post-evento

**2. Negociación de subvenciones**

- Presentación de dossiers de financiación ante el CA del BDI con escenarios A/B/C de venta e inversión
- Resultado: Expo Bingo 2026 — 300 € adjudicados sobre 500 € solicitados

**3. Organización de eventos**

- Co-responsable de la Expo Bingo en World Week 2026 (evento principal del mandato): negociación con el Club Chileno por el espacio del musée, coordinación con ADR y SBCS, gestión de proveedores, ejecución el día del evento y revisión post-mortem estructurada

#### Competencias demostradas

Gestión financiera en sistema bancario y regulatorio francés (facturación con SIRET, cheques de caution, conciliación de gastos, rendición de cuentas) · Negociación presupuestaria con órgano institucional de aprobación · Gestión de eventos end-to-end con equipos de voluntarios · Stakeholder management multi-actor (BDI, ADR, BDE, clubes culturales internacionales) · Onboarding y liderazgo de comunidad internacional · Transferencia de conocimiento institucional

> **[META-INSTRUCCIÓN]** Esta entrada debe aparecer siempre junto al rol de Secretario General del BDI, y viceversa: juntos justifican la dimensión asociativa completa del perfil.

---

### Delegado de Clase (3 años consecutivos) — Universitat Politècnica de València (UPV)

**Tipo:** Vida asociativa — representación estudiantil electa
**Período:** 2020 – 2023
**Ubicación:** Valencia, España

#### Contexto del rol

Representante académico electo durante tres cursos consecutivos de un grupo de ~80 compañeros de la promoción de Ingeniería Industrial de la UPV. Funciones: canal de comunicación entre alumnos y profesorado, recogida sistemática de feedback con propuestas concretas a la administración, mediación en situaciones de conflicto excepcionales.

#### Episodios concretos

**Episodio 1 — Reforma del máster** Lideró la iniciativa que resultó en la reestructuración de tres asignaturas troncales durante el proceso de reforma del máster. Implicó recogida estructurada de feedback del alumnado, elaboración de propuestas concretas y negociación con la dirección de la escuela.

**Episodio 2 — DANA de Valencia (2024)** Gestionó el conflicto de fechas de exámenes generado por la DANA para seis estudiantes con plaza Erasmus en riesgo de perderla. Tres rondas de negociación con profesores, dirección de escuela y los propios alumnos hasta alcanzar una solución que respetase la integridad académica sin perjudicar a ningún estudiante.

#### Competencias demostradas

Liderazgo transversal sin autoridad jerárquica · Negociación multi-actor · Mediación en conflicto bajo restricción temporal y presión emocional · Comunicación con autoridades académicas · Articulación y defensa de intereses colectivos · Resiliencia y orientación a resultados en situaciones excepcionales

> **[META-INSTRUCCIÓN]** Los episodios 1 y 2 son material STAR para entrevistas y pueden explotarse de forma independiente como respuestas a preguntas de liderazgo, negociación o gestión de conflictos.

---

## SECCIÓN 6 — PROYECTOS E INNOVACIÓN

---

### Aplicación de optimización de staff planning para asociaciones — Pôle Projet Associatif Numérique, CentraleSupélec

**Tipo:** Proyecto de innovación — modelado matemático, desarrollo de software, transformación digital
**Período:** diciembre 2024 – en curso
**Cliente / Contexto:** Pôle Projets Associatifs Numériques de CentraleSupélec. Proyecto autogenerado, admitido como proyecto oficial del semestre. Primera usuaria de producción: Lisa Devaux (Secretaria General del BDI en el ski de invierno del BDI).
**Equipo:** Individual (proyecto en solitario)
**Rol individual:** Concepción, formulación matemática, desarrollo, user research, iteración y coordinación con la responsable del Pôle Projet (Jennifer) para la puesta en producción

#### Problema de negocio identificado

El staff planning es uno de los procesos operativos más tediosos y costosos en las asociaciones estudiantiles de CentraleSupélec. El proceso heredado: encuesta (LettuceMeet/Google Forms/WhatsApp) → Excel → resolución manual. Coste: entre 1 hora y decenas de horas de trabajo activo por evento, con resultados frecuentemente subóptimos (distribuciones inequitativas, asignaciones no deseadas, ausencia de descansos estructurados).

#### Formulación matemática del modelo (MILP)

Programa lineal entero mixto (MILP) multi-objetivo sobre cuatro índices: personas × tareas × horas × días. El modelo incorpora un conjunto amplio de parámetros operativos — disponibilidad (dura y blanda), demanda por tarea, competencias, preferencias, restricciones de rotación y vínculos sociales — junto con una variable binaria principal de asignación y un conjunto de variables auxiliares para penalizar violaciones de restricciones con distinta severidad. La función objetivo minimiza una suma ponderada con jerarquía explícita, priorizando cobertura de demanda y mandatos del responsable sobre equidad, preferencias individuales y continuidad de turno.

#### User research y metodología de desarrollo

- Más de 20 entrevistas semidirigidas con secretarios generales de asociaciones de CentraleSupélec: BDI, BDA, BDE, WACS, RAID, TOSS, PICS, Huma, NCV, Forum, Sympo, AdR, Capese, Genius, Nepal, OSER, entre otros
- Objetivo: mapear la diversidad de reglas operativas de cada asociación y traducirlas a nuevas restricciones formales del modelo
- Backlog activo con priorización must-have/nice-to-have, refactorización iterativa, corrección de bugs, optimización de uso de RAM

#### Decisión metodológica sobre AI-assisted development

La formulación matemática y el diseño del modelo son aportación humana íntegra. La implementación de la interfaz Python y la lógica de aplicación se aceleraron mediante Claude Code y herramientas equivalentes de desarrollo asistido por IA. Esta decisión eliminó meses de aprendizaje en front-end que no eran el cuello de botella intelectual del proyecto. Es un caso concreto y cuantificable de AI-augmented engineering: criterio humano sobre el "qué" y el "por qué", aceleración asistida por IA sobre el "cómo".

#### Resultados y entregables

- Reducción del tiempo de planificación: de 1–10 horas de trabajo manual a segundos–minutos de cómputo automatizado (reducción de ~2 órdenes de magnitud)
- Aplicación funcional desplegada y validada en uso real
- Modelo matemático documentado y reutilizable
- Más de 20 entrevistas a stakeholders convertidas en restricciones operativas configurables
- Hoja de ruta activa con must-haves y nice-haves priorizados
- Adopción comprometida por la totalidad de las asociaciones de CentraleSupélec una vez completado el despliegue online
- Proyectos derivados ya identificados: modelos específicos para RAID y Forum; algoritmo de matching para el programa de mentoring del BDI

#### Competencias demostradas

_Técnica y cuantitativa:_ Investigación operativa aplicada · Mixed-integer linear programming · Modelado matemático multi-objetivo · Linearización de restricciones no convexas · Solver comercial Gurobi · Arquitectura de aplicaciones web · Optimización de recursos computacionales (RAM, warm starts)

_Transformación digital y AI-assisted engineering:_ Diseño de workflows híbridos humano-IA en proyecto real con usuarios finales · Descomposición del problema entre componentes que requieren juicio humano y componentes susceptibles de delegación a IA · Criterio para validar output de IA contra especificación formal previa · Caso cuantificable de uso de IA para multiplicar productividad individual

_Producto y consultoría:_ User research estructurado a escala (20+ entrevistas) · Traducción de requisitos ambiguos a especificaciones formales · Stakeholder management con perfiles heterogéneos · Identificación de oportunidades de transformación digital en procesos manuales aceptados como inevitables

_Ejecución y autonomía:_ Iniciativa propia (proyecto autogenerado, no asignado) · Entrega sostenida bajo restricciones de tiempo no remuneradas

---

### Misión de consultoría de operaciones H2E Supply Chain — Air Liquide / ST7 Optimisation et gestion de flux, CentraleSupélec

**Tipo:** Proyecto — consultoría de operaciones con cliente industrial real
**Período:** abril 2026
**Cliente / Contexto:** Air Liquide (CAC 40, líder mundial en gases industriales). Marco: asignatura ST7 de CentraleSupélec con interlocución directa con el cliente. Dominio: mobilité hydrogène / cadena de suministro de hidrógeno en modo swap.
**Equipo:** internacional multicultural (Brasil, China, Portugal, España)
**Rol individual:** Líder técnico del proyecto. Responsable del desarrollo de la heurística de despacho (componente algorítmico más diferenciado) y de la construcción del fichero base de simulación en Simul8. Contribución transversal a: diseño de experimentos, razonamiento económico de recomendaciones, redacción del informe, preparación y entrega de la presentación final ante Air Liquide.

#### Problema planteado por el cliente

Red de distribución en estrella: un Centro de Llenado (FC) con 3 rampas paralelas abastece 5 Estaciones de Repostaje de Hidrógeno (HRS) a distancias de 50–200 km, con tasas de consumo heterogéneas (10–50 kg/h) y operativa de 20h/día. Operación en modo swap. Restricciones: turnos de conductores (05:00–13:00 y 13:00–21:00), ventana nocturna sin despachos, prohibición dominical de entregas.

**Pregunta central del cliente:** ¿Cuál es la configuración mínima viable de flota, conductores y capacidad de almacenamiento que garantiza cero roturas de stock?

#### Metodología y herramientas

**Modelo de simulación de eventos discretos (Simul8):** Cada remolque transita entre estados: parking vacío → rampa de llenado → parking lleno → tránsito ida → swap en HRS → tránsito vuelta. Recursos limitantes: operador del FC (24/7) y pool de conductores (turnos). Componentes estocásticos: tiempos de conexión/desconexión Uniform[3,8] min y tiempos de tránsito Normal con σ = 10% de la media.

**Heurística de despacho reactiva y greedy (diseño propio del equipo, contribución de Óscar):** En cada ciclo de evaluación, el algoritmo calcula una función de coste compuesta para cada estación elegible:

- Coste de tránsito (tarifa de conductor × tiempo)
- Penalización por ineficiencia (kg no absorbibles porque el tanque sigue demasiado lleno, valorados al precio de mercado del H₂)
- Penalización por horas extra (si el retorno excede el fin de turno)
- Componente de ruptura (bonus fuertemente negativo cuando el nivel proyectado en llegada cae bajo el margen de seguridad dinámico; penalización de superávit que desincentiva entregas prematuras)
- Margen de seguridad dinámico según turno y día: el turno nocturno del sábado incorpora un suplemento de 24h para garantizar supervivencia del domingo bajo prohibición de entregas

Esta heurística es propietaria del equipo y trasciende el uso de funcionalidades preconfiguradas de Simul8.

**Protocolo de validación (buenas prácticas de simulación):**

- Período de calentamiento: determinado por método gráfico con filtro de media móvil (k=10); fijado conservadoramente en 60 horas
- Número de réplicas: calculado a IC 95% mediante calculadora de Simul8 (4 réplicas)
- Validación white-box: seguimiento manual de remolques, verificación de niveles de H₂, prueba de condiciones extremas (N=0/M=0 vs. N=50/M=50), balance de masa en múltiples instantes

**Experimentación:** Reducción secuencial de recursos desde configuración base hasta mínimo viable en 3 escenarios (HRS tanque único, HRS multi-tanque, restricción dominical). Análisis de sensibilidad sobre 5 escenarios.

#### Entregables al cliente

- Informe técnico estructurado (metodología, validación, experimentación)
- Modelo Simul8 reutilizable
- Tablas de configuraciones óptimas y análisis de sensibilidad con intervalos de confianza
- Recomendación final argumentada económicamente

#### Competencias demostradas

_Técnica y cuantitativa:_ Simulación de eventos discretos · Diseño de heurísticas de despacho propietarias con función de coste multi-componente · Modelado estocástico de cadenas de suministro logísticas · Validación estadística de modelos (warm-up, réplicas, IC 95%, white-box) · Análisis de sensibilidad sistemático · Dimensionado de flota y recursos · Visual Logic (Simul8)

_Operaciones y consultoría:_ Fleet dimensioning · Optimización de cadenas de suministro logísticas · Gestión de restricciones operativas complejas (turnos, ventanas temporales, restricciones regulatorias) · Análisis coste-beneficio de configuraciones alternativas · Formulación de KPIs operativos acordados con cliente · Bottleneck dynamics · Recomendaciones accionables con justificación económica explícita

_Cliente y soft:_ Interacción directa con cliente industrial CAC 40 (Air Liquide) en dominio estratégico (transición energética / H₂) · Reuniones intermedias de alineación · Presentación final de resultados al cliente · Liderazgo técnico individualizable dentro del equipo · Redacción de informe técnico orientado a decisor industrial · Trabajo en equipo internacional multicultural

---

### 1.er Premio — Hackathon Estudio de Caso, Programa Akademia — Fundación Innovación Bankinter (GENAQ)

**Tipo:** Proyecto — consultoría estratégica en formato hackathon nacional competitivo
**Período:** septiembre 2024 – febrero 2025
**Cliente / Contexto:** GENAQ (fabricante del grupo Keyter, especializado en generación de agua atmosférica). Empresa pivotando de B2B consolidado (Departamento de Defensa de EE.UU., Acciona, Meliá) a mercado doméstico B2C/B2B con nuevo producto residencial (PVP 2.500 €, 10–12 L/día, vida útil 20 años).
**Marco competitivo:** Hackathon nacional del programa Akademia (Fundación Innovación Bankinter/UPV). 80 equipos competidores. Jurado ejecutivo externo.
**Equipo:** 6 personas
**Rol individual:** Líder del equipo. Responsable de la concepción del sistema de parámetros (arquitectura de la función multicriterio, selección y ponderación de los 7 criterios, lógica de normalización). Responsable de la presentación final ante el jurado (narrativa estratégica completa: del buyer persona a la recomendación de canal, pasando por el roadmap de producto). Coordinación de contribuciones individuales en torno a una tesis única. Trazabilidad entre input (datos públicos), modelo (función ponderada) y output (50 secciones priorizadas + recomendación accionable).

**Validación externa:** 🏆 1.er Premio entre 80 equipos · Jurado ejecutivo de la Fundación Bankinter

#### Problema de negocio

GENAQ disponía de tecnología validada en segmento industrial pero carecía de estrategia de entrada para su nuevo producto residencial. Pregunta: "¿cómo lanzamos este MVP en España?" — formulación ambigua que requería traducción a problema operacionalizable.

**Entregables requeridos:** (1) Identificación geográfica del mercado piloto al máximo nivel de granularidad (sección censal, ~36.000 unidades en España) + (2) formulación de la estrategia comercial asociada.

#### Metodología

**Modelo MCDA (Multi-Criteria Decision Analysis):** 7 dimensiones ponderadas para scoring de secciones censales:

1. Pirámide poblacional — 30%
2. Renta y consumo del hogar en categoría COICOP de electrodomésticos — 20%
3. Gasto en agua embotellada — 20%
4. Rendimiento y coste de generación según temperatura y humedad relativa (curvas propietarias del cliente) — 10%
5. Coste unitario del agua corriente — 10%
6. Densidad de población — 5%
7. Pluviometría — 5%

Pesos calibrados a partir del buyer persona definido previamente (30–50 años, ingreso medio-alto, entorno urbano/suburbano, sensibilidad ambiental).

**Fuentes de datos:**

- INE: renta media por hogar, encuesta de presupuestos familiares, suministro y saneamiento del agua
- AEMET: precipitación provincial
- Datos propietarios del cliente: curvas de generación-coste según condiciones ambientales

**Normalización:** 0-100 para combinar variables heterogéneas. Geo-visualización a nivel de sección censal sobre todo el territorio nacional.

**Capa adicional — Algoritmo K-Means no supervisado:** 5 clusters sobre las dos variables de mayor peso (renta y pirámide poblacional), diseñado como evolución del modelo para segmentar el mercado más allá del top-50 y soportar futuras iteraciones de marketing.

**Decisión de canal:** Análisis comparativo cualitativo con 5 criterios (experimentación previa a la compra, redes de distribución existentes, presencia online, cobertura nacional, servicio al cliente) sobre 3 alternativas: e-commerce propio, tienda física propia, grandes superficies. Resultado: distribuidores mayoristas en grandes superficies, con Leroy Merlin como partner prioritario por capacidad técnica de instalación.

**Herramienta entregable:** Modelo Excel automatizado y reutilizable con pesos editables por el cliente para regenerar resultados bajo distintos escenarios estratégicos.

#### Resultados

- Lista priorizada de 50 secciones censales óptimas, top 10 identificado; concentración en el arco mediterráneo (Cataluña, Comunidad Valenciana) con nichos en Madrid, Aragón y zonas pirenaicas
- Recomendación de canal: grandes superficies, Leroy Merlin como partner prioritario
- Roadmap de mejoras de producto: IoT, agua caliente, agua con gas, integración fotovoltaica, rediseño estético
- Modelo paramétrico reutilizable entregado al cliente como herramienta operativa, no como informe estático
- 🏆 1.er Premio entre 80 equipos

#### Competencias demostradas

_Analítica y técnica:_ Estructuración de problemas de negocio ambiguos · Modelado cuantitativo aplicado a decisiones estratégicas · Análisis multicriterio (MCDA) · Normalización y consolidación de fuentes heterogéneas · Geospatial analytics · Clustering no supervisado (K-Means) · Construcción de herramientas de decisión reutilizables en Excel

_Consultoría y negocio:_ Market entry strategy · Customer segmentation · Buyer persona · Go-to-market design · Análisis de canales con trade-off matrix · Roadmap de producto · Comunicación ejecutiva y data storytelling

_Liderazgo:_ Dirección de equipo multidisciplinar de 6 personas · Integración de contribuciones técnicas en narrativa única · Pitch ante jurado externo bajo presión competitiva · Gestión de plazos en formato hackathon

---

### Proyecto de mejora operativa y logística — Veganic

**Tipo:** Proyecto — consultoría operativa con cliente industrial real (PYME)
**Período:** febrero 2025 – junio 2025
**Cliente / Contexto:** Veganic — PYME industrial del sector agroquímico (biocontrol / bioinsecticidas y biofungicidas), polígono de Paterna, Valencia. Empresa de 7 meses de antigüedad al inicio del proyecto, con crecimiento acelerado e ineficiencias operativas estructurales. Contacto obtenido por relación personal (fundador Carlos Ledó Orriach, amigo del padre de Óscar). Trabajo final de la asignatura Diseño e Instalación de Equipos Industriales del Máster en Ingeniería Industrial de la UPV (curso 2024-25).
**Equipo:** 3 personas
**Rol individual:** Client lead — el cliente fue conseguido a través de contacto personal propio. Responsabilidades: gestión directa de la relación con el cliente, coordinación de la visita presencial (7 de abril de 2025), entrevistas con el fundador (Carlos Ledó Orriach) y con el responsable de almacén y logística (Alonso García Pérez), recopilación in situ de información técnica sobre maquinaria, flujos y problemáticas. Contribución técnica transversal: diagnóstico operativo, diseño de propuestas de mejora, cálculos técnicos (dimensionado rampa de muelle, mesa de rodillos, velocidad de paletizado), elaboración del modelo VAN.

#### Contexto de la empresa cliente

Veganic: 7 meses en operación al inicio del proyecto. Desarrolla, fabrica y comercializa bioinsecticidas y biofungicidas en España y mercados internacionales. Nave industrial de 1.242 m² distribuidos en dos naves adosadas. Crecimiento acelerado con ineficiencias heredadas de decisiones tomadas sobre la marcha.

**5 debilidades críticas identificadas en el diagnóstico:**

1. Desorganización del inventario (sin criterios de clasificación)
2. Desaprovechamiento del 40% del espacio de estanterías (ocupado por palets de cartón)
3. Cuello de botella en el pasillo principal
4. Rampa de acceso peligrosa al 15% de pendiente
5. Paletizado manual con riesgo ergonómico

#### Metodología

**Fase 1 — Levantamiento de información:** Visita presencial a la nave. Observación directa del proceso productivo. Entrevistas semiestructuradas con fundador y responsable de almacén. Documentación fotográfica del layout y los flujos.

**Fase 2 — Diagnóstico:** Identificación de los 5 problemas críticos arriba descritos.

**Fase 3 — Generación de propuestas:** 2 sesiones de brainstorming en Microsoft Teams aplicando la técnica SCAMPER (Sustituir, Combinar, Adaptar, Modificar, Eliminar, Reordenar). Criterios de selección: urgencia/impacto, viabilidad técnica, relación con la materia.

**Fase 4 — Validación técnica y económica:**

- Dimensionado técnico de equipos: cálculo trigonométrico de longitud mínima de rampa de muelle (inclinación ≤3% / ≤6%), cálculo del paso entre rodillos y carga máxima por rodillo de la mesa transportadora (33 rodillos, paso 64 mm, 4,27 kg/rodillo), fórmulas de velocidad de paletizado en bultos/hora con y sin sistema mecanizado
- Clasificación ABC del inventario
- Elaboración de presupuestos detallados por partidas (PEM, gastos indirectos, beneficio industrial, IVA)
- Análisis VAN con horizonte de 24 meses, tipo de descuento del 10% e inflación

#### Las 5 propuestas de mejora (sinérgicas entre sí)

|#|Propuesta|Presupuesto|
|---|---|---|
|1|Muelle de carga con foso interior + rampa móvil al 3% + eliminación selectiva de estanterías para abrir ruta alternativa|34.355 €|
|2|Habilitación del patio frontal exterior (28,8 m²) como zona cubierta de almacenaje (techado ligero)|4.533 €|
|3|Ampliación de pared de estanterías con bastidores Mecalux 2.625 mm: aprovechamiento del espacio del 60% al 90%|1.650 €|
|4a|Mejora del paletizado — opción bajo coste: transpaleta elevable|899 €|
|4b|Mejora del paletizado — opción avanzada: paletizadora automática SocoSystem|21.200 €|
|5|Reorganización del inventario con clasificación ABC|Sin coste de equipo|

#### Resultados cuantitativos

- Incremento de productividad en paletizado: de 1.125 a 1.894 bultos/hora (+68%)
- VAN de la paletizadora automática: amortización en 18 meses incluso bajo tipo de descuento del 10% (ahorro de un salario mensual neto de costes energéticos)
- Aprovechamiento del espacio de estanterías: de 60% a 90%

#### Entregable

Informe técnico-económico de 48 páginas: memoria técnica, cálculos, presupuestos, discusión y conclusiones. Propuestas diseñadas para generar sinergias entre sí. Alineación con ODS (Metas 8.2, 8.8 y 9.1) con definición de KPIs de seguimiento.

#### Competencias demostradas

_Técnico-operacional:_ Diagnóstico de procesos productivos in situ · Comprensión de flujos logísticos en nave industrial · Dimensionado técnico de equipos · Lectura de catálogos de proveedores (Mecalux, SocoSystem, Conveyor Belts Ibérica) · Elaboración de presupuestos por partidas · Modelado financiero (VAN, payback, sensibilidad a tipo de descuento) · Clasificación ABC en gestión de inventarios · Ergonomía industrial y normativa de seguridad laboral

_Consultoría:_ Relación directa con cliente real · Entrevistas a múltiples niveles (dirección y operaciones) · Identificación de quick wins vs. inversiones estratégicas · Formulación de propuestas con varias opciones de inversión para que el cliente decida según su apetito de capex · Técnicas de creative problem solving (SCAMPER) · Articulación de informe que combina rigor técnico con argumentación de coste de oportunidad

_Transversal:_ Account management con cliente real · Alineación con marco de sostenibilidad (ODS) · Comunicación profesional escrita y visual (planos, diagramas de flujo, tablas presupuestarias)

---

### Análisis de viabilidad nacional del sistema Bokashi — Pôle Projet S7, CentraleSupélec

**Tipo:** Proyecto — estudio de viabilidad multidisciplinar (consultoría de política pública / sostenibilidad)
**Período:** septiembre 2025 – febrero 2026
**Cliente / Contexto:** Pôle Projet S7 de CentraleSupélec. Contexto regulatorio: ley francesa AGEC (Anti-Gaspillage pour une Économie Circulaire) vigente desde 1 de enero de 2024, que obliga a separación en origen de biorresiduos para todos los productores.
**Equipo:** 5 personas (5 nacionalidades distintas)
**Rol individual:** Contribución transversal al diseño del scope, la estructuración del análisis multicriterio y la elaboración del informe final (42 páginas). Aportaciones específicas: diseño del marco de identificación de barreras (risk mapping en 6 dimensiones); conducción y síntesis de la entrevista con RECUP' Occitanie (Toulouse); experimentación de campo de 2 meses con sistema Bokashi doméstico; análisis comparativo con la alternativa de metanización metropolitana en Toulouse.

#### Contexto regulatorio

Desde el 1 de enero de 2024, la ley AGEC obliga a la separación en origen de biorresiduos para todos los productores (hogares, empresas, ERP). A mediados de 2024, solo el 47% de la población tenía acceso real a una solución de recogida selectiva. Bokashi (fermentación anaerobia con microorganismos eficaces) se presentaba como posible solución descentralizada para entornos urbanos densos.

**Pregunta del estudio:** ¿Puede Bokashi actuar como solución de gestión de biorresiduos a escala nacional en Francia?

#### Metodología (5 bloques)

1. **Análisis legal y regulatorio:** Código de Medio Ambiente francés (artículos L541-21-1 a L541-21-5) + comparación con la regulación alemana (BioAbfV) para el caso de contaminación de residuos
2. **Modelo cuantitativo de viabilidad económica a nivel hogar:** Inversión inicial (3 contenedores, 120–160€) + costes recurrentes (activador comercial vs. casero, con desglose de ingredientes y proporcionalidad mensual basada en datos ADEME e INSEE)
3. **Carbon accounting completo:** Emisiones por fermentación + producción del salvado + transporte (Australia–Francia) + manufactura de 12,2 M de contenedores plásticos a escala París → 146.400 t CO₂ total
4. **Revisión científica crítica:** Contraste de Zhang 2022 vs. Mayer 2010 sobre beneficios agronómicos; detección de sesgos en los claims de los estudios pro-Bokashi
5. **Primary research:** Entrevista semiestructurada con RECUP' Occitanie + experimentación de campo de 2 meses con documentación fotográfica del proceso

**Herramientas y frameworks:** Análisis multicriterio · Cost-benefit analysis · Life cycle assessment simplificado · Benchmarking internacional (caso alemán) e intersectorial (Bokashi vs. compostaje aerobio vs. metanización) · Revisión bibliográfica (36 referencias) · Entrevista cualitativa estructurada · Experimentación participante · Redacción final en LaTeX en inglés

#### Resultados cuantitativos

- Coste de adopción por hogar: 130–160€ de inversión inicial + ~70€/año recurrentes
- Huella de carbono operativa de Bokashi: 58 kg CO₂/t de residuo (prácticamente equivalente a los 62 kg del compostaje tradicional — anula la ventaja teórica del proceso anaerobio)
- Capacidad teórica de absorción agrícola: 28–42 Mt/año vs. 4,14 Mt/año de producción potencial (sobrada, pero limitada por estacionalidad y concentración en agricultura ecológica: 2,8 Mha sobre 14,2 Mha aptas)
- Barreras reales identificadas: gestión del lixiviado y fricción cotidiana del usuario

#### Conclusión estratégica

**Bokashi NO es viable como solución nacional.** Debe relegarse a rol complementario en nichos urbanos muy específicos (París: saturación del espacio público, regulación patrimonial que impide contenedores en calle). La solución sistémica recomendada es la **metanización integrada en infraestructuras de tratamiento de aguas residuales existentes** — modelo ya operativo en Toulouse Métropole (planta Ginestous-Garonne, proyecto Energibio): menor fricción para el usuario, producción de digestato SPAn3 directamente utilizable, recuperación energética vía inyección de biometano en la red.

#### Competencias demostradas

_Técnico-analítico:_ Feasibility study multicriterio sobre problema abierto · Modelado cuantitativo bottom-up (costes, volúmenes, emisiones) · Carbon accounting y comprensión de cadenas logísticas y agronómicas · Lectura crítica de literatura científica con detección de sesgos · Análisis comparativo de tecnologías y benchmarking internacional

_Consultoría:_ Definición de scope · Identificación temprana de hipótesis a validar · Integración de fuentes primarias y secundarias · Recomendación accionable con argumentación estructurada

_Soft y transversal:_ Trabajo en equipo multicultural (5 nacionalidades) · Comunicación con stakeholders externos (entrevista a RECUP' Occitanie) · Redacción técnica en inglés · Comprensión de transformación regulatoria con impacto cross-sectorial (residuos, agricultura, energía, política pública, comportamiento del consumidor)

> **[META-INSTRUCCIÓN]** El origen de este proyecto es un marcador de criterio proactivo: Óscar negoció con el profesor redirigir el Pôle Électronique (asignado por defecto) hacia un estudio de viabilidad de consultoría, al considerar que el ángulo pertinente era cuestionar si los esfuerzos de implementación electrónica merecían la pena. Esta historia puede explotarse en entrevistas como demostración de pensamiento crítico y autonomía. La story condensada para entrevistas es: "Estudiamos si Bokashi podía ser solución nacional, terminamos recomendando lo contrario y reorientando hacia metanización" — demuestra criterio analítico, no solo capacidad de ejecución.

---

### Regulación de potencia de aerogenerador CART — Enseignement d'Intégration Systèmes Multi-Énergie, CentraleSupélec

**Tipo:** Proyecto académico — modelado, control óptimo y análisis energético
**Período:** noviembre 2025
**Cliente / Contexto:** Asignatura Enseignement d'Intégration: Systèmes Multi-Énergie de CentraleSupélec. Turbina de referencia: CART (Controlled Advanced Research Turbine), descrita en literatura científica de L2S/CentraleSupélec y EDF R&D.
**Equipo:** 5 personas — Óscar Martínez Zamora (primero en la portada, en lista no ordenada alfabéticamente), Aziz Kaboré, Isabela Corrêa Hillal, Rafael Kenji Teramoto, Beatriz Brioli Doll de Moraes
**Rol individual:** Primer nombre en la portada del equipo de 5 (lista no ordenada alfabéticamente).

#### Alcance técnico del proyecto

El proyecto cubre el ciclo completo de ingeniería de control de un aerogenerador CART:

**1. Modelado no lineal:** Sistema de 3 ecuaciones diferenciales acopladas (dinámica del rotor, del generador y de la torsión del eje). Coeficiente de potencia Cp(λ, θ) según modelo aerodinámico de la turbina CART.

**2. Análisis cualitativo de estabilidad:** Estudio del signo de ∂Tr/∂ωr en el entorno de los puntos de equilibrio. Distinción de zonas estables (el sistema absorbe perturbaciones) de zonas inestables (el sistema las amplifica). Resultado clave: el modelo no lineal sin control se detiene completamente con vientos bajos por entrada en zona inestable — fenómeno que el modelo lineal no reproduce por construcción (identificación honesta y bien interpretada de comportamiento no trivial).

**3. Linealización:** Desarrollo de Taylor de primer orden alrededor del punto de equilibrio óptimo (ωr,opt = 3,05 rad/s, λopt = 9,2, θ = 5°). Cálculo simbólico de jacobianos → matrices A, B, Bd, C, D en representación de estado.
**Protocolo de validación original:** Aplicación de escalones de couple de amplitudes variables (de 10⁻⁶ a 10² N·m) y verificación de que el error de modelado escala cuadráticamente con la amplitud (E ∝ Δũ²). Pendiente 2 en escala log-log = prueba experimental de que el modelo está bien construido.

**4. Diseño de controlador LQI (Linear Quadratic Integral):** Regulación cuadrática lineal con acción integral sobre el error de velocidad rotórica. Matrices de ponderación QLQI y RLQI ajustadas iterativamente para cumplir: sobreimpulso < 10%, tiempo de respuesta < 20 s, error estático nulo. Gain K obtenido resolviendo la ecuación algebraica de Riccati asociada mediante función `lqr` de MATLAB. Controlador resultante: K = 10³·[−2,18; −0,041; 0,407; −0,548].

**5. Optimización numérica de parques eólicos en cascada:** Modelado del efecto de estela según teoría de momentum de Betz. Corrección crítica identificada por el equipo: la formulación inicial vsiguiente = v(1−α) genera rendimientos no físicos (>100% para 7+ turbinas); sustituida por vout = vin(1−2α), que respeta la conservación de la energía. Optimización del vector de factores de inducción {αi} con `fmincon` de MATLAB para 2, 3 y 4 turbinas.

#### Resultados clave

- Identificación del punto de operación absolutamente óptimo: Cp,max = 0,4655 con λopt = 8,14 y θopt = 0° (frente a Cp,max = 0,3476 con la restricción θ = 5° del enunciado)
- El control LQI **duplica** la energía extraída respecto a la operación sin control (32,1% del potencial Betz vs. 16,0%), con perfil de viento real (wind7.mat)
- Rendimientos globales de cascadas: 2 turbinas = 64,00% · 3 turbinas = 65,31% · 4 turbinas = 65,84%
- Análisis físico de retornos marginales decrecientes en cascadas

#### Episodios clave

1. **Autocorrección del modelo de cascada:** el equipo detecta que su formulación viola la conservación de la energía (rendimiento >100% con 7+ turbinas), diagnostica la causa y corrige. Secuencia clásica de consultor: pensamiento crítico → diagnóstico → corrección con marco teórico.
2. **Interpretación del fenómeno de detención del modelo no lineal:** el aparente "error" numérico es la manifestación correcta de la zona inestable predicha en la sección 2. Capacidad de conectar resultados numéricos con teoría física.
3. **Validación experimental cuadrática del error de linealización:** diseñar un experimento que confirma una predicción teórica de orden de magnitud. Metodología científica de manual.

#### Competencias demostradas

_Técnica y científica:_ Modelado no lineal de sistemas dinámicos multivariable · Análisis cualitativo de estabilidad · Linealización por Taylor con validación experimental rigurosa · Control óptimo (LQR/LQI) · Tuning de matrices de ponderación frente a especificaciones cuantitativas · Optimización numérica con restricciones (fmincon) · Simulación en MATLAB/Simulink nivel avanzado · Análisis energético comparativo con marco teórico explícito (límite de Betz, teorema de momentum)

_Razonamiento técnico transferible a consultoría:_ Identificación y corrección de errores de modelado mediante consistencia física · Interpretación física de fenómenos numéricos contraintuitivos · Articulación de trade-offs entre escala y rendimiento marginal en sistemas multi-componente · Dominio del lenguaje técnico de transición energética y energías renovables

> **[META-INSTRUCCIÓN]** Los tres episodios clave pueden explotarse de forma independiente en entrevistas, con independencia del rol exacto asumido en el proyecto. Son especialmente pertinentes para demostrar autocorrección, interpretación física de fenómenos numéricos y metodología científica rigurosa.

---

### Análisis crítico del paper "Portage and Path Dependence" (Bleakley & Lin, QJE 2012) — Économie de la Croissance, CentraleSupélec

**Tipo:** Trabajo académico — revisión crítica individual de paper económico de primer nivel
**Período:** febrero 2026
**Cliente / Contexto:** Asignatura Économie de la Croissance del Máster en CentraleSupélec. TD en grupo de 3 estudiantes estructurado como literature review colectiva sobre persistencia de desigualdades regionales (3 papers seminales: Dell 2010 en Econometrica, Donaldson 2018 en AER, Bleakley & Lin 2012 en QJE).
**Equipo:** 3 personas
**Rol individual:** Autoría única del análisis del paper de Bleakley & Lin (apartado 3, dos páginas densas en formato de revisión académica). Coautoría de la introducción y de la conclusión integradora del trabajo (síntesis de los tres mecanismos en un marco unificado de self-reinforcing feedback loops). Elección del paper y selección del ángulo crítico final: aportaciones propias.

#### Contexto del paper analizado

_Portage and Path Dependence_, Bleakley & Lin, Quarterly Journal of Economics 2012 (ranking CNRS 1e). Estudia cómo los puntos de portaje fluvial en la colonización de Norteamérica (que dieron origen a ciudades como Filadelfia, Baltimore, Washington D.C. o Chicago) siguen organizando la actividad económica actual a través de economías de aglomeración y dependencia del sendero, incluso cuando la justificación material original ha desaparecido (reemplazada por canales, ferrocarriles y nuevas fuentes de energía en el siglo XIX).

**Pregunta de investigación del paper:** ¿Cómo se identifican las ventajas geográficas obsoletas que persisten a través de coordination inertia frente a las que se diluyen con el tiempo?

#### Estructura del análisis realizado (5 bloques)

1. Presentación del problema de identificación
2. Descripción del fenómeno histórico del portaje
3. Exposición secuencial de los 4 modelos econométricos del paper con resultados cuantitativos principales:
- Validación de la persistencia con datos censales 1790–2000 y luminosidad nocturna de 2003
- Identificación del mecanismo de comercio mediante el upstream watershed
- Análisis de evolución temporal con difference-in-differences
- Descarte de la hipótesis alternativa de sunk costs mediante precios de vivienda y salarios
4. Discusión del mecanismo causal: coordination device vs. sunk costs
5. Reflexión personal con apertura a debate sobre optimalidad económica de las ciudades persistentes y el rol del capital humano

**Resultados cuantitativos del paper:** Proximidad a un sitio de portaje predice +50–110% en densidad poblacional actual. Coeficiente DiD positivo de 0,456 que refuta la reversión a la media.

**Aportación crítica original:** Hipótesis propia sobre el capital humano como verdadero durable sunk asset de una ciudad, conectando los hallazgos de Bleakley & Lin (mayor crecimiento poblacional en sitios de portaje con mayores tasas de alfabetización en el siglo XIX) con la evidencia comparativa de Davis & Weinstein (2002) sobre la reconstrucción de ciudades japonesas tras la Segunda Guerra Mundial.

#### Competencias demostradas

Lectura crítica de literatura económica de máximo nivel (QJE, CNRS 1e) · Comprensión de métodos econométricos avanzados (regresión discontinua geográfica, DiD, análisis contrafactual) · Dominio del vocabulario de economía del desarrollo y geografía económica (path dependence, agglomeration economies, coordination failures, sunk costs) · Síntesis y comunicación técnica en inglés académico · Articulación de argumento original a partir de literatura existente · Pensamiento crítico aplicado a formulación de políticas públicas

---

## SECCIÓN 7 — FORMACIÓN COMPLEMENTARIA

---

### Programa Akademia Future Builders — Fundación Innovación Bankinter / Universitat Politècnica de València

**Tipo:** Formación complementaria — programa extracurricular competitivo con ejecutivos en activo
**Período:** septiembre 2024 – febrero 2025

#### Descripción del programa

Akademia Future Builders es un programa de formación complementaria en innovación organizado por la Fundación Innovación Bankinter en colaboración con la UPV, dirigido exclusivamente a estudiantes con potencial diferencial de universidades seleccionadas en España (UPV, Comillas, USC, Loyola, entre otras). Acceso por proceso de selección competitivo — Óscar fue admitido en la convocatoria de lunes de la edición UPV 2024.

**Formato:** 21,5 horas de sesiones en formato debate con directivos y expertos (no clase magistral) — requería preparación autónoma previa de informes sectoriales enviados por la organización y participación activa con preguntas y reflexiones propias.

#### Programa y contenidos (7 áreas temáticas)

|Área|Contenido|Experto/Organización|
|---|---|---|
|Innovación|Cómo se generan y escalan ideas transformadoras en entornos corporativos|Fernando Alfaro|
|Energía|Descarbonización, transición energética e impacto de la IA en la demanda energética global|Fernando Torrico (Iberdrola)|
|Agua|Sostenibilidad hídrica e innovación en gestión del agua|Álvaro Silva|
|Telecomunicaciones|Evolución del 5G y su rol como infraestructura habilitadora de nuevas industrias|Juan Cambeiro (Telefónica)|
|Datos|Análisis de datos y toma de decisiones en entornos de alta complejidad|María Iglesias (Viralgen)|
|IA y Computación Cuántica|Tendencias emergentes y potencial disruptivo en los sectores anteriores|—|
|Visita a empresa|Jeanología — tecnología de acabado textil; inmersión en entorno de innovación industrial real|Jeanología|

#### Aprendizajes clave

- Capacidad para leer e interpretar megatendencias tecnológicas y traducirlas en implicaciones estratégicas concretas para empresas e industrias
- Transición energética desde perspectiva de negocio: descarbonización, nuevas fuentes, impacto de la IA en la demanda, modelos emergentes de distribución
- 5G y telecomunicaciones de nueva generación como infraestructura habilitadora de transformaciones industriales más amplias
- Datos como activo estratégico; modelos de toma de decisiones basados en análisis cuantitativo en sectores regulados y biotecnológicos
- Sostenibilidad hídrica como vector de innovación con implicaciones tecnológicas, regulatorias y geopolíticas
- Marco mental para identificar oportunidades en la intersección entre sectores

#### Competencias adquiridas

Technology landscape analysis (contextualización de tecnologías emergentes en su impacto sectorial) · Cross-sector strategic thinking (conexión de disrupciones entre sectores aparentemente no relacionados) · Executive dialogue readiness (exposición a debates con directivos de Iberdrola, Telefónica y Grupo Santander) · Self-directed learning (preparación autónoma de materiales técnicos sectoriales fuera del aula) · Innovation framing (análisis de tendencias desde perspectiva de generación de oportunidades, no solo de gestión del riesgo)

> **[META-INSTRUCCIÓN]** Mientras la carrera aportaba rigor técnico y analítico, Akademia proporcionó perspectiva estratégica sobre los sectores transformados por la tecnología — dimensión que la formación académica reglada raramente cubre con esta profundidad ni con estos interlocutores. Esta entrada es especialmente pertinente para responder en entrevistas preguntas de tipo "¿qué tendencia tecnológica te parece más disruptiva y por qué?"

---

## SECCIÓN 8 — MISCELÁNEA (activos de archivo)

> **[META-INSTRUCCIÓN DE SECCIÓN]** Los proyectos de esta sección tienen veredicto de importancia muy baja en el análisis del corpus. Se documentan por completitud y como reserva narrativa para entrevistas. Su uso en CV está sujeto a las meta-instrucciones indicadas en cada entrada.

---

### Encargado eléctrico/mecánico y coordinador logístico — Sardinia Innovative Boat Week

**Tipo:** Miscelánea — proyecto de ingeniería extracurricular, competición universitaria
**Período:** octubre 2024
**Ubicación:** Cerdeña, Italia

#### Contexto

Competición anual en Cerdeña en la que universidades de toda Europa presentan embarcaciones eléctricas tripuladas construidas durante el curso. La edición en la que participó Óscar fue la primera en que un equipo español fue seleccionado para competir en la historia del concurso.

#### Rol y responsabilidades

- **Equipo:** 8 personas
- Dos roles en paralelo:
1. Responsable del equipo eléctrico y mecánico de la embarcación: contribución al diseño y montaje de los subsistemas eléctricos y mecánicos
2. Coordinador logístico del proyecto: garantía de que cada componente llegase a Cerdeña a tiempo y listo para montar, gestión de imprevistos del transporte internacional
- Terminaron la construcción del barco in situ en Cerdeña y compitieron en la prueba

#### Hito notable

Primera participación española en la historia del concurso (primera vez que un equipo español fue seleccionado).

#### Competencias demostradas

Capacidad técnica electromecánica aplicada · Coordinación logística internacional · Trabajo en equipo bajo presión de calendario · Resolución de problemas técnicos en condiciones no controladas · Iniciativa extracurricular por amor a la ingeniería

> **[META-INSTRUCCIÓN]** La narrativa correcta para usar esta experiencia si se incluye NO es el logro competitivo (el equipo compitió para participar, no para ganar). La narrativa es la pasión por la ingeniería más allá del currículum académico: "lo hicimos por amor al arte, por el placer puro de construir algo con las manos."

---

### Mención Honorífica — Concurso Nacional "Jóvenes Investigadores"

**Tipo:** Miscelánea — concurso de investigación al inicio de la trayectoria universitaria
**Período:** septiembre 2020
**Referencia oficial:** NIPO 130-21-006-0

#### Descripción

Concurso nacional de investigación dirigido a estudiantes de últimos cursos de bachillerato y primeros de universidad. Presentación pública ante jurado.

**Resultado:** Mención Honorífica — top 10% entre 40 equipos competidores. Premio de 1.000 €.

#### Rol

Líder del equipo de 4 personas. Coordinación del cronograma del trabajo de investigación, delegación de tareas, integración de las contribuciones individuales y exposición final ante el jurado.

#### Competencias demostradas

Liderazgo temprano de equipo · Coordinación y delegación · Comunicación oral ante tribunal externo · Gestión de proyecto con plazo y entregable · Primera experiencia formal de liderazgo y exposición competitiva ante tribunal externo

> **[META-INSTRUCCIÓN]** El Premio Akademia hackathon (1.er Premio entre 80 equipos, jurado ejecutivo, feb 2025) compite directamente con esta entrada y lo supera en todos los parámetros. Incluir solo si hay espacio y el formato lo permite.
