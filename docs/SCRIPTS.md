...
Validates that all prerequisites are in place: Node.js >= 18, dependencies installed, Playwright chromium, required files (`config/profile.md`, `config/portals.yml`), fonts directory, and auto-creates `data/`, `output/reports/`, `output/CVs/`, `output/interview-prep/` if missing.
...
Health check for pipeline data integrity. Validates `data/applications.md` against seven rules: canonical statuses (per `states.yml`), no duplicate company+role pairs, all report links point to existing files, scores match `X.XX/5` / `N/A` / `DUP`, rows have proper pipe-delimited format, no pending TSVs in `batch/tracker-additions/`, and no markdown bold in scores.
...
Maps non-canonical statuses to their canonical equivalents and strips markdown bold and dates from the status column. Aliases like `Sent` become `Applied`, `Closed` becomes `Discarded`, etc. Duplicate info is moved to the notes column.
...
Validates that the career-ops setup is internally consistent: `config/profile.md` exists with required fields and CV body sections, no hardcoded metrics in `modes/_shared.md` or `batch/batch-prompt.md`, and `config/profile.md` freshness (warns if older than 30 days).
...
Applies the upstream update. Creates a backup branch (`backup-pre-update-{version}`), fetches from the canonical repo, checks out only system-layer files, runs `npm install`, and commits. User-layer files (`config/profile.md`, `data/`, etc.) are never touched.
