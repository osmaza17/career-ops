...
<purpose>
Builds a company- and role-specific interview preparation report. Triggered when the user asks to prep for an interview at a specific company+role, or when an evaluation scores 4.0+ and the user updates status to `Interview`. Outputs a structured intel report covering process overview, round breakdown, likely questions, technical checklist, and company signals.
</purpose>
...
| 2 | `output/reports/` | Evaluation report (if exists) — archetype, gaps, matched proof points |
| 3 | `config/profile.md` (body) | CV sections — proof points |
| 4 | `config/profile.md` (YAML frontmatter + `strategy.*`) | Candidate context, narrative, gaps |
</reference>
...
Questions about leadership, conflict, collaboration, failure. For each: question, source, and the candidate's best angle based on their CV proof points.
</section>
...
<step id="5" name="Technical prep checklist">
<agent_instruction>
...
<step id="6" name="Company signals">
...
<output>
Save the full report to `output/interview-prep/{company-slug}-{role-slug}.md`.
</output>

<mapping>
File: `output/interview-prep/{company-slug}-{role-slug}.md`
